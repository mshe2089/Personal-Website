...
def getmymarks(username, password):
    ...
    """
    try:
        coursetabletext = (driver.find_elements_by_class_name("sv-panel-primary")[3]\
                           .find_element_by_tag_name("tbody")).text
        ...
        WAM_tot = sum([float(i[4])*float(i[6]) if not (i[4].isspace() or i[6].isspace()) else 0 for i in courselist])
        GPA_tot = sum([WAM2GPA(float(i[4]))*float(i[6]) if not (i[4].isspace() or i[6].isspace()) else 0 for i in courselist])

        WAM = str(round(WAM_tot/CP_tot, 2))
        GPA = str(round(GPA_tot/CP_tot, 2))
    except:
        driver.quit()
    ...
    driver.quit()
    return(coursetabletext, WAM, GPA)
